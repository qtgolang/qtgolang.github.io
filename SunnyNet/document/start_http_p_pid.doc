...
.如果 (Sunny中间件.启动 ())
    ...
    ' 若是还不行，建议尝试进程名 http://esunny.vip/ 中的 [Sunny网络中间件-示例文档]->[（抓HTTP/HTTPS）指定进程名]
    启动线程 (&遍历pid, , )
    信息框 (“启动成功”, 0, , )
...
.判断循环首 (真)
    ...
    .计次循环首 (取数组成员数 (Array), index)
        ' 判断进程名 是否为需要拦截的进程名
        ' 到小写 避免大小写问题
        .如果真 (寻找文本 (到小写 (Array [index].进程名称), 到小写 (“qq.exe”), , 假) ≠ -1)
            ' 判断之前有没有添加过
            .如果真 (寻找文本 (已添加, 到文本 (Array [index].进程标识符) ＋ #换行符, , 假) ＝ -1)
                ' 添加PID     进程标识符=pid
