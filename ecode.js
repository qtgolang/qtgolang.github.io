		...
		ecode.trans = function (ele) { //ele如果为空则转换当前所有class为ecode的，ele为指定的即转换指定，同时ele可以为自定义的元素数组
			...
			for (var a = 0; a < eleEcode.length; a++) {
				...
				if (eleEcode[a].getAttribute("status") != "trans") {
					...
					eleEcode[a].style.height = eleEcode[a].clientHeight + 50 + "px";
...
async function EcodeCopyCode(a) {
	...
	if (eleP.querySelector(".origiData").style.display == "block") {
		...
		if (typeof loadScript === "function") await loadScript("web/box.js");
		if (typeof loadCSS === "function") await loadCSS("web/sw.css");
		if (copyToClipboard(eleP.getElementsByTagName("textarea")[0].innerHTML) == false) {
			eleP.querySelector(".origiData").style.display = "block";
			a.innerHTML = "恢复视图";
			if (typeof swal === "function") swal("复制失败", "请手动复制", "error"); else alert("复制失败，请手动复制");
			return;
		} else {
			if (typeof swal === "function") swal("已复制", "请打开易语言粘贴", "success"); else alert("已复制到剪贴板");
		}
	}
}
...
(function () {
	...
	function adjustLineElements() {
		var list = document.querySelectorAll('.ecode-wrapper .line1, .ecode-wrapper .line2, .ecode-wrapper .line3, .ecode-wrapper .line4');
		for (var i = 0; i < list.length; i++) {
			var el = list[i];
			if (el.getAttribute('is') === '1') continue;
			var topVal = parsePx(el.style.top);
			if (isNaN(topVal)) topVal = parsePx(getComputedStyle(el).top) || 0;
			var heightVal = parsePx(el.style.height);
			if (heightVal<40){continue;}
			if (isNaN(heightVal)) heightVal = parsePx(getComputedStyle(el).height) || 0;
			el.style.top = (topVal + 10) + 'px';
			el.style.height = Math.max(0, heightVal - 20) + 'px';
			el.setAttribute('is', '1');
		}
		requestAnimationFrame(adjustLineElements);
	} 
	requestAnimationFrame(adjustLineElements);
	if (document.readyState === 'loading') {
		document.addEventListener('DOMContentLoaded', startAdjustInterval);
	}
})();
